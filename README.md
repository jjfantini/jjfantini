...
- **agent orchestration (private clients)** 🤖 - fault-tolerant LLM pipelines with typed outputs, RL feedback loops, retry mechanisms, observability, and distributed agent architectures built to handle concurrent AI operations. this is an evergreen project, always iterating and improving. it is my personal investment strategy.

**past projects, in detail:**

- **[Finatic](https://finatic.dev)** - co-founded and served as Chief Engineer. built the cloud-native multi-broker trading backend (FastAPI + Supabase/PostgreSQL + Redis) from scratch - real-time trading, high-throughput API workloads, DB-centric automated type generation across Python and TypeScript, and an API designed to be consumed natively by modern AI protocols (MCP, A2A, ACP).
- **[humblFINANCE](https://humblfinance.io)** - elegant website and UX, enabling retail traders access to professional hedge-fund grade quantamental analytics; built from an pricing algorithm I developed (humblDATA). full-stack: database, user management, API integration, custom quant algorithm for stable stock price analysis and macro forecasting.

---

**three architecture patterns I keep coming back to:** 🏗️

**OOOO (Observe, Orient, Output, Optimize)** - this is an internal framework that I developed to spin of of the classic OODA loop. with a focus on software, this guides my initial research into solving any probelm. first, understand the landscape. second, ideate a plan to solve the problem within the constraints. third, generate and deliver the result. fourth, when the deliverable is on target, iterate and improve upon it.
...
`Python 🐍` `TypeScript🗳️` `R 🔬` `Rust 🧱` `Go ➡️`  `FastAPI⚡️` `Next.js` `React ⚛️` `PostgreSQL 🧮` `Redis 💽` `Polars 🐻‍❄️` `Docker 🐋` `Supabase 🚀` `DigitalOcean 🌊` 
